...
# total input feature dimension: 9 physchem + 5 structural + 3 hse = 17
IN_DIM = 17


# ── parsing ───────────────────────────────────────────────────────────────────

def compute_structural_features(coords):
    """coords: (N, 3) numpy array of Ca positions → (N, 5) structural features"""
    N = len(coords)
    ...
    for i in range(N):
        ...
        # local density — number of Ca within 8A (excl. self)
        features[i, 0] = (dists < 8.0).sum() - 1

        # approximate burial — inverse mean distance to 5 nearest neighbors
        nearest = np.sort(dists)[1:6]
        features[i, 1] = 1.0 / (nearest.mean() + 1e-8)

        # secondary structure from Ca geometry
        if 2 <= i <= N - 3:
            ...
            is_helix = float(3.6 < d_prev < 4.0 and 3.6 < d_next < 4.0 and cos_angle > 0.5)
            is_sheet = float(d_prev > 4.5 and d_next > 4.5 and cos_angle < -0.3)
            is_loop  = float(not is_helix and not is_sheet)
        else:
            is_helix, is_sheet, is_loop = 0.0, 0.0, 1.0  # terminal → loop

        features[i, 2] = is_helix
        features[i, 3] = is_sheet
        features[i, 4] = is_loop

    return features


def compute_hse(coords):
    """
    Approximate half-sphere exposure from Ca coordinates only.
    For each residue, split neighbors within 13A into 'up' and 'down'
    relative to the local chain direction vector.
    Returns (N, 3): [hse_up, hse_down, hse_ratio]
    hse_ratio = hse_down / total — high means buried/concave (likely pocket)
    """
    N = len(coords)
    ...
    for i in range(N):
        ...
        if i == 0:
            chain_vec = coords[1] - coords[0]
        elif i == N - 1:
            chain_vec = coords[-1] - coords[-2]
        else:
            chain_vec = coords[i + 1] - coords[i - 1]
        chain_vec = chain_vec / (np.linalg.norm(chain_vec) + 1e-8)

        # neighbors within 13A
        dists = np.linalg.norm(coords - coords[i], axis=1)
        neighbor_mask = (dists < 13.0) & (dists > 0)
        neighbors = coords[neighbor_mask] - coords[i]

        hse_up, hse_down = 0.0, 0.0
        for n in neighbors:
            if np.dot(n, chain_vec) > 0:
                hse_up += 1
            else:
                hse_down += 1

        total = hse_up + hse_down + 1e-8
        features[i, 0] = hse_up
        features[i, 1] = hse_down
        features[i, 2] = hse_down / total  # concavity score
...
def parse_mol2_protein(path):
    ...
    # normalise each feature group across residues
    def norm(x):
        std = x.std(axis=0)
        std[std < 1e-8] = 1.0
        return (x - x.mean(axis=0)) / std

    h = np.concatenate([norm(physchem), norm(structural), norm(hse)], axis=1)  # (N, 18)
...
class ScPDBDataset(Dataset):
    def __init__(self, root_dir: str, cache_workers: int = 8, threshold: float = 1.0):
        ...
        # pre-load everything into memory — parsing mol2 on the fly during training is too slow
...
def split_dataset(dataset, val_frac=0.1, test_frac=0.1, seed=42):
    ...
    # group by PDB ID so all chains of the same protein end up in the same split
...
class PocketFinder(nn.Module):
    ...
    def forward(self, h, pos, batch):
        ...
        # build knn graph on Ca positions; edge attribute is Euclidean distance
        edge_index = knn_graph(pos, k=10, batch=batch, loop=False)
        row, col = edge_index
        edge_attr = (pos[row] - pos[col]).norm(dim=-1, keepdim=True)

        # residual connections help a lot with deeper networks
...
def evaluate(model, subset, device, beta=2.0):
    # macro-averaged: compute per-protein then average
    model.eval()
...
def main():
    ...
    best_f2   = 0.0
    best_loss = float("inf")
    epochs_without_improvement = 0  # for early stopping — tracked separately from best_f2
