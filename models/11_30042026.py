...
# total input feature dimension: 9 physchem + 6 structural = 15
IN_DIM = 15
...
def parse_mol2_protein(path):
    ...
    # normalise each feature across residues
    def norm(x):
        std = x.std(axis=0)
        std[std < 1e-8] = 1.0
        return (x - x.mean(axis=0)) / std

    h = np.concatenate([norm(physchem), norm(structural)], axis=1)  # (N, 15)
