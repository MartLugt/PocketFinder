...
# total input feature dimension: 9 physchem + 6 structural + 3 hse = 18
IN_DIM = 18
...
def compute_structural_features(coords):
    """coords: (N, 3) numpy array of Cα positions → (N, 6) structural features"""
    N = len(coords)
    ...
    for i in range(N):
        ...
        # local density — number of Cα within 8Å (excl. self)
        ...
        # secondary structure from Cα geometry
        if 2 <= i <= N - 3:
        ...
        features[i, 3] = is_helix
        features[i, 4] = is_sheet
        features[i, 5] = is_loop
...
def compute_hse(coords):
    """
    Approximate half-sphere exposure from Cα coordinates only.
    For each residue, split neighbors within 13Å into 'up' and 'down'
